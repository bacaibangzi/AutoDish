...
<body>
    ...
    <div class="contenbox">
    <c:if test="${vo.orgType <2}">
	</c:if>
    	<div class="grid_summay_opts" id="btns">
				<div class="left_opts">
						<a hideFocus="true" href="javascript:add('<%=basePath%>agOrg/eidt.htm?code=${vo.code}&orgType=${vo.orgType}')" class="gray_radiu_btn" >
							<em class="gray_l"></em> 
							<em class="gray_r"></em> 
							新增
						</a>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			var tableObj={}
			tableObj.url="<%=basePath%>agOrg/list.htm?code=${vo.code}";
		...
		function update()
		{
			...
			window.location = "<%=basePath%>agOrg/eidt.htm?code=${vo.code}&orgCode="+orgCode+"&entityId="+selectId+"&orgType="+orgType; 
		}
		/** 查看详情 **/
		function detail()
		{
			...
			var selectId = data.idStr;
			// 明细弹出框
			var commonDialog = commonOpenDialog("readDetail",'详细',700,450, '<%=basePath%>agOrg/detail.htm?code=${vo.code}&entityId=' + selectId);
		...
		function highQuery()
		{
			...
			var returnVal = '<%=basePath%>agOrg/list.htm?code=${vo.code}';
		...
		function deleteAll()
		{
			...
			var flag = null;
			$.ajax({
				url:'<%=basePath%>agOrg/checkIfCanbeDeleted.htm',
				type:'post',
				async: false,
				data:'entityIds='+ids,
			...
			var rows=getSeleteObjs(datatable);
			var actionUrl = "<%=basePath%>agOrg/delete.htm?code=${vo.code}&entityIds=";
