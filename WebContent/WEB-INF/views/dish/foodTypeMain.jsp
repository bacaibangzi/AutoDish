...
<body>
    ...
    <div class="contenbox">
    	<div class="grid_summay_opts" id="btns">
				<div class="left_opts">
						<a hideFocus="true" href="javascript:add('<%=basePath%>foodType/eidt.htm?orgCode=${sessionScope.accountInfo.orgCode}')" class="gray_radiu_btn" >
							<em class="gray_l"></em> 
							<em class="gray_r"></em> 
							新增
						</a>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			var tableObj={}
			tableObj.url="<%=basePath%>foodType/list.htm?orgCode=${sessionScope.accountInfo.orgCode}";
		...
		function update()
		{
			...
			window.location = "<%=basePath%>foodType/eidt.htm?entityId="+selectId; 
		...
		function detail()
		{
			...
			var selectId = data.idStr;
			// 明细弹出框
			var commonDialog = commonOpenDialog("readDetail",'详细',700,450, '<%=basePath%>foodType/detail.htm?entityId=' + selectId);
		...
		function highQuery()
		{
			// 查询条件 区域名称
			var areaName_filter = '';
			var returnVal = '<%=basePath%>foodType/list.htm?orgCode=${sessionScope.accountInfo.orgCode}';
			if(areaName_filter!=''){
		...
		function deleteAll()
		{
			...
			if(rows.length!=1){
				...
				var actionUrl = "<%=basePath%>foodType/delete.htm?code=${vo.code}&entityIds=";
