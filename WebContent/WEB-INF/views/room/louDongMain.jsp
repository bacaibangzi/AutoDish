...
<body>
    ...
    <div class="contenbox">
    	<div class="grid_summay_opts" id="btns">
				<div class="left_opts">
						<a hideFocus="true" href="javascript:add('<%=basePath%>louDong/eidt.htm?orgCode=${sessionScope.accountInfo.orgCode}')" class="gray_radiu_btn" >
							<em class="gray_l"></em> 
							<em class="gray_r"></em> 
							新增
						</a> 
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			var tableObj={}
			tableObj.url="<%=basePath%>louDong/list.htm?orgCode=${sessionScope.accountInfo.userId}";
			...
			tableObj.aoColumns=[
				...
				{"mDataProp":"no","sDefaultContent": "","bSortable":false,"sClass":"text-align-mid","sWidth":"200"},
		...
		function update() {
			...
			window.location = "<%=basePath%>louDong/eidt.htm?entityId="+selectId; 
		...
		function detail() {
			...
			var selectId = data.idStr;
			// 明细弹出框
			var commonDialog = commonOpenDialog("readDetail",'详细',700,450, '<%=basePath%>louDong/detail.htm?entityId=' + selectId);
		...
		function highQuery() {
			// 查询条件 区域名称
			var areaName_filter = '';
			var returnVal = '<%=basePath%>louDong/list.htm?orgCode=${sessionScope.accountInfo.orgCode}';
			if(areaName_filter!=''){
		...
		function deleteAll() { 
			...
			if(rows.length!=1){
				...
				var rows=getSeleteObjs(datatable);
				var actionUrl = "<%=basePath%>louDong/delete.htm?code=${vo.code}&entityIds=";
